...
const PointsGive = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const confirmMessage = `${selectedUserNames}에게 ${parseInt(pointAmount).toLocaleString()}P를 지급하시겠습니까?`;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-gray-600">선택한 회원들에게 포인트를 일괄 지급할 수 있습니다.</p>
      </div>
              ...
              {selectedUsers.length > 0 && pointAmount && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <div className="text-sm text-blue-800 space-y-1">
                    ...
                    <div>개별 지급액: {parseInt(pointAmount || 0).toLocaleString()}P</div>
                    <div className="font-medium">총 지급액: {totalAmount.toLocaleString()}P</div>
                  </div>
                </div>
              )}
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-yellow-800 space-y-1">
          <li>• 포인트 지급은 즉시 반영되며, 취소할 수 없습니다.</li>
          <li>• 지급 내역은 포인트 내역에서 확인할 수 있습니다.</li>
          <li>• 지급 사유는 회원에게도 표시됩니다.</li>
          <li>• 대량 지급 시 시스템 처리에 시간이 걸릴 수 있습니다.</li>
        </ul>
      </div>
    </div>
  );
